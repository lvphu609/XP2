...
class Notifications extends Rest_Controller
{
     ...
     * set notify is viewed
     * @method: Post
     * @link access: http://domain/xproject/api/notifications/delete
     *
     * parram
     * @token   string has
     *
    ...
    function delete_post(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //update notify read at
            $isDelete = $this->notification->delete($this->account_info);
            if($isDelete){
                $status = API_SUCCESS;
                $message = '';
            }
     ...
     * set notify is viewed
     * @method: Post
     * @link access: http://domain/xproject/api/notifications/get_notify_number
     *
     * parram
     * @token   string has
     *
     ...
     * @return array
     */
    function get_notify_number_post(){
        $status = API_FAILURE;
        $message = API_ERROR;
        $results = null;
        $validation = null;
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //update notify read at
            $number = $this->notification->getNotifyNumber($this->account_info);
            $status = API_SUCCESS;
            $message = '';
            $results = $number;
        }
